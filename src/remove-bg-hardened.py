...
def rgb_to_ycbcr(
    r: NDArray[np.floating], g: NDArray[np.floating], b: NDArray[np.floating]
...
def detect_background_color(image_path: Path) -> tuple[int, int, int]:
    ...
    try:
        ...
        corners.extend(
            tuple(px)
            for px in data[0:sample_size, w - sample_size : w, :].reshape(-1, 3)
        )
        corners.extend(
            tuple(px)
            for px in data[h - sample_size : h, 0:sample_size, :].reshape(-1, 3)
        )
        corners.extend(
            tuple(px)
...
def remove_background(
    input_path: Path, output_path: Path | None = None, autocrop: bool = False
) -> Path:
    """
